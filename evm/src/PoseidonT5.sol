...
// Vendored from `poseidon-solidity` v0.0.5 (MIT, https://github.com/vimwitch/poseidon-solidity)
// — Circom-compatible Poseidon over BN254 (x^5 S-box), same parameter family
// `light-poseidon` (off-chain prover) and `sol_poseidon` (Solana syscall) already
// implement (OPAQ.md A.7). Byte-parity with both verified in evm/test/Poseidon.t.sol
// before OpaqPool.sol trusts it (B.12.9's EVM Poseidon parity gate) — do not edit by
// hand; regenerate via the same package if parameters ever need to change.

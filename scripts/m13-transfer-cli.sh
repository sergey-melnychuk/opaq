#!/usr/bin/env bash
# M13 / Phase 2 P2.4 (OPAQ.md B.4.3): drive the full private loop from the `opaq`
# CLI — deposit -> transfer -> withdraw the change — end-to-end on a validator.
#
# NOTE: insecure test ceremony (see OPAQ.md B.6) — exercises the CLI/transfer flow.
set -euo pipefail
...
WORK="$(mktemp -d)"
export OPAQ_PASSPHRASE="opaq-m13-test-passphrase"
...
# Fixed deposit/transfer/withdraw zkeys + VKs (setup is circuit-only, so the CLI's
# own freshly-minted notes prove against them).
...
solana-test-validator --reset --quiet --ledger "$WORK/ledger" >/tmp/opaq-m13-validator.log 2>&1 &
VPID=$!
...
OPAQ_ROOT="$ROOT" \
node "$ROOT/tests/m13_transfer_cli.mjs" "$PROG_KP" "$WORK/mint.json" "$OPAQ"
